...
const Dashboard = () => {
    ...
    return (
        <Box paddingBottom={10} paddingLeft={10}>
            <Heading fontSize={'8vh'} align='center' justify='center' p={8}>Dashboard</Heading>

            <Search onChange={handleInputChange} input={input} setInput={setInput} />

            <Works data={projects.filter((data) =>
                (jaroWinklerSimilarity(data.name, input) >= THRESHOLD ||
                jaroWinklerSimilarity(data.description, input) >= THRESHOLD) || input === ""
            )} />
        </Box>
    )
}
